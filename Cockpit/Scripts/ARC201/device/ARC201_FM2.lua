...
local paramFreq = get_param_handle("ARC201_FM2_FREQ") -- this is for FM Homing?
local paramDisplayFreq = get_param_handle("ARC201_FM2_FREQ_DISPLAY") -- this is for the radio display
...
dev:listen_command(Keys.fm2SRSrx) -- this "key" is pressed by SRS when receiving on this radio
...
function SetCommand(command,value)
    ...
    elseif command == device_commands.fm2ModeSelector then
    ...
    elseif command == Keys.fm2SRSrx then -- this should only ever be done by SRS
    ...
    elseif command == Keys.ptt and math.floor(GetDevice("PLT_ICP"):get_argument_value(400) * 5) == 4 then -- the SRS PTT keybind and ICP selector set to radio 4
        if isScanMode then
            scanPTT(value)
        end
    else
...
function loadFH(key)
    ...
    elseif canEnterData then
        if key == "CLR" and displayString == tostring(presets[presetMode + 10]) then
            countdownTimer = 7 --reset timer upon any keypress
...
function setFrequency(newFrequency)
    ...
    FM2paramFreq:set(newFrequency) -- set freq for SRS
	--paramFreq:set(newFrequency) -- set freq for FM Homing
    --print_message_to_user(FM2paramFreq:get())
end
